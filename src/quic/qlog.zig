...
/// Legacy JSON-SEQ header fields kept alongside `file_schema_uri` /
/// `serialization_format` purely for external-tool compatibility: qvis's
/// hosted `.sqlog` loader (`TextSequenceJSONToQlog`, as of its `QlogSchema02`
/// active schema) hard-rejects any json-seq header lacking `qlog_version` and
/// `qlog_format: "JSON-SEQ"`, even though the file-schema-URN pair above is
/// what current qlog drafts actually key the format on (#625).
...
pub fn writeTraceHeader(header: TraceHeader, out: []u8) error{NoSpaceLeft}![]const u8 {
    ...
    try b.add("{{\"file_schema\":\"{s}\",\"serialization_format\":\"{s}\",\"qlog_version\":\"{s}\",\"qlog_format\":\"{s}\",\"title\":", .{ file_schema_uri, serialization_format, legacy_qlog_version, legacy_qlog_format });
...
test "trace header satisfies qvis's json-seq header check (#625)" {
    ...
    // Confirmed against a real trace generated by h3_interop_tool and loaded
    // into https://qvis.quictools.info/.
    var buf: [1024]u8 = undefined;
    const header = try writeTraceHeader(.{ .vantage_point = .client, .group_id = "ac75373edb249fe6" }, &buf);

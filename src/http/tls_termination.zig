//! Native TLS termination surface for every shipping profile
//! (#379, epic #327, #634, retirement #649).
//!
//! Imported directly by every consumer of TLS termination — `http.zig`'s
//! public alias and the `src/http/` call sites — so exactly one backend
//! type set exists in the build. Contains no `@cImport`, no OpenSSL types,
//! and no C linkage in either `-Dtls-profile=general` or
//! `-Dtls-profile=appliance`: both are native-only. Downstream TLS is
//! served by the native listener (`native_tls_connection.zig`), never by
//! this file; any attempt to construct the OpenSSL-shaped `TlsTerminator`
//! fails closed with `error.ContextInitFailed` — a deliberate, inspectable
//! failure rather than a hidden runtime fallback. (Historically this file
//! held that terminator's OpenSSL implementation and a `_stub.zig` twin
//! carried this native surface; #649 retired the OpenSSL implementation
//! entirely and promoted the native surface to this file's name.)
//!
...
//!
//! Option structs retain the field shapes the retired OpenSSL adapter used
//! so configuration parsing behaves identically across profiles; only the

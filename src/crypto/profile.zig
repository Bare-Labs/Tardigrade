...
pub const rows = [_]Row{
    ...
    row(.{ .signature = .ecdsa_secp256r1_sha256 }, "ECDSA-P256-SHA256", .zig_std_crypto, .supported, .openssl_provider, .provider_deferred, "SEC1 key/DER signature verify, provider.SigningKey sign/verify, deterministic and failing signing entropy, tamper, wrong-key, non-canonical-signature rejection (#343/#432)", .{ .{ .tls_handshake, .live }, .{ .pki, .live } }, .{ .native_appliance, .general_purpose_openssl }, "Same split as Ed25519, now both live (#490/#432): PKI chain verification and CertificateVerify both call CryptoProvider.verify, and local signing goes through provider.SigningKey with per-signature noise from injected provider entropy; the secp256r1 ECDH group remains deferred regardless."),

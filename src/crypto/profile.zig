...
pub const rows = [_]Row{
    ...
    row(.{ .signature = .ecdsa_secp256r1_sha256 }, "ECDSA-P256-SHA256", .zig_std_crypto, .supported, .openssl_provider, .provider_deferred, "SEC1 key/DER signature verify, tamper, wrong-key, non-canonical-signature rejection (#343)", .{ .{ .tls_handshake, .live }, .{ .pki, .live } }, .{ .native_appliance, .general_purpose_openssl }, "Same split as Ed25519, now both live (#490): PKI chain verification and CertificateVerify both call CryptoProvider.verify, and local signing goes through provider.SigningKey; the secp256r1 ECDH group remains deferred regardless."),
    row(.{ .signature = .rsa_pss_rsae_sha256 }, "RSA-PSS-RSAE-SHA256", .project_code, .supported, .openssl_provider, .provider_deferred, "strict DER/RSA-PSS positive and negative fixtures", .{ .{ .tls_handshake, .not_integrated }, .{ .pki, .live } }, .{.general_purpose_openssl}, "Unlike Ed25519/ECDSA, this stays not_integrated for tls_handshake deliberately (#490): the native TLS 1.3 engine's CertificateVerify negotiates only Ed25519/ECDSA-P256, so RSA-PSS is never selected there even though the provider primitive exists and PKI chain verification already uses it — primitive support does not imply protocol integration. Pure-Zig verifier validates the complete EMSA-PSS encoding and constrained RSA public keys; the native appliance negotiates Ed25519/ECDSA only."),
    ...
    row(.{ .entropy = .injected_random_bytes }, "injected random bytes", .project_code, .supported, .openssl_provider, .provider_deferred, "deterministic and failing entropy tests", .{ .{ .tls_handshake, .live }, .{ .quic_packet_protection, .not_integrated }, .{ .resumption, .not_integrated } }, .{ .native_appliance, .general_purpose_openssl }, "CryptoProvider.generateKeyShare draws its ephemeral X25519 scalar from CryptoProvider.entropy.fill (#490), so the TLS 1.3 handshake engine is now live; QUIC packet protection and resumption/ticket code still inject randomness through their own longer-standing Entropy parameters instead, in both product profiles. Randomness changes require no-ambient-RNG review regardless of which seam supplies it."),
...
test "native TLS 1.3 handshake engine is live-integrated through CryptoProvider" {
    ...
    try expectIntegration(.{ .signature = .rsa_pss_rsae_sha256 }, .tls_handshake, .not_integrated);
}

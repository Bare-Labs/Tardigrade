...
/// enabled for `.native_appliance`, while `.general_purpose` can still
/// enable it under a different product policy even when
/// `.openssl_status == .provider_deferred` (that field tracks whether an
/// in-process OpenSSL `CryptoProvider` exists — it never has — independent
/// of which product profile enables the capability).
pub const ProductProfile = enum {
    ...
    /// `.native_appliance` — the two profiles differ in product policy, not
...
test "row helpers expose consumers and enabled product profiles" {
    ...
    for (rows) |entry| {
        ...
        try std.testing.expect(entry.enabled_product_profiles.contains(.general_purpose));
    }
...
// the group pending #391, so it must not claim `.native_appliance` —
...
// since #649), so this row must still claim `.general_purpose`.
test "enabled product profiles are not a mechanical copy of primitive support" {
    ...
    try std.testing.expect(aes_256_profiles.contains(.general_purpose));
    ...
    try std.testing.expect(chacha_profiles.contains(.general_purpose));
    ...
    try std.testing.expect(secp256r1_profiles.contains(.general_purpose));
    ...
    try std.testing.expect(rsa_pss_profiles.contains(.general_purpose));
    ...
    try std.testing.expect(rsa_pkcs1_sha256_profiles.contains(.general_purpose));
    ...
    try std.testing.expect(rsa_pkcs1_sha384_profiles.contains(.general_purpose));
    ...
    try std.testing.expect(!header_protection_profiles.contains(.general_purpose));
}

...
pub const rows = [_]Row{
    ...
    row(.{ .hkdf = .sha384 }, "HKDF-SHA384", .zig_std_crypto, .supported, .openssl_provider, .provider_deferred, "provider capability and expand-label tests", .{.{ .tls_handshake, .not_integrated }}, .{.general_purpose_openssl}, "src/tls/key_schedule.zig calls std.crypto directly; the native appliance negotiates TLS_AES_128_GCM_SHA256 only, so HKDF-SHA384 is not selected there. HKDF additions require TLS 1.3 label coverage."),

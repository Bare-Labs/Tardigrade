...
test "enabled product profiles are not a mechanical copy of primitive support" {
    ...
    // #649: `general` is native too and drives the same in-process QUIC/H3
    // stack as `appliance`, so QUIC header protection is live in both — this
    // is no longer an asymmetric row like secp256r1/RSA-PSS/RSA-PKCS1 above.

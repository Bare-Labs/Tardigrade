...
test "countLocationBlocks includes top-level location blocks, not just server-block ones" {
    // Regression: `tardi check`/`print-config` reported "location blocks: 0"
    // for every profile generated by `tardi init` (static, proxy, ...),
    // because countLocationBlocks only summed server_blocks[].location_blocks
    // and ignored the config's own top-level location_blocks -- the field
    // every `tardi init` profile actually populates, since none of them wrap
    // their routes in an explicit `server { ... }` block.
    const allocator = std.testing.allocator;
    try std.testing.expectEqual(@as(c_int, 0), setenv("TARDIGRADE_LOCATION_BLOCKS", "exact|/health|return|200|ok", 1));
    defer _ = unsetenv("TARDIGRADE_LOCATION_BLOCKS");

    var cfg = try edge_config.loadFromEnv(allocator);
    defer cfg.deinit(allocator);

    try std.testing.expectEqual(@as(usize, 1), cfg.location_blocks.len);
    try std.testing.expectEqual(@as(usize, 1), countLocationBlocks(&cfg));
}

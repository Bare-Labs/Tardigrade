...
| `src/quic/tls_adapter.zig` | provider-owned keyed crypto | Migrating. Provider-backed HKDF, AEAD, and AES header-protection APIs exist (`*WithProvider`). Existing concrete wrappers remain as a temporary compatibility exception for call sites not yet carrying a provider handle. |

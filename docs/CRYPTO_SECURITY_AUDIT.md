...
| `src/crypto/rsa.zig` (`PrivateKey`, `signPssSha256`) / `pure_zig.SoftwareRsaSigningKey` (#565) | RSA private exponent `d` (retained); modulus `n` (retained, public); `p`, `q`, `dp`, `dq`, `qInv` (parsed and range-validated at `parsePrivateKeyDer`, then discarded — not retained in `PrivateKey`); the 32-byte PSS salt drawn from `provider.Entropy` per signature | secret (`d`, salt); public (`n`); transient-secret, not retained (`p`/`q`/`dp`/`dq`/`qInv`) | sign (EMSA-PSS encode + private-exponent modular exponentiation), store | not observable (the signature is the only output; a wrong/malformed key or entropy failure never publishes a partial signature — enforced by both `signPssSha256`'s ordering, see below, and `SoftwareRsaSigningKey.signingSignImpl`'s output-length check before the entropy draw, mirroring `SoftwareEcdsaP256SigningKey`) | `PrivateKey.deinit` (`secrets.secureZero(&self.d)`) wipes the retained private exponent; `SoftwareRsaSigningKey.deinit` delegates to it; `FixedCredentialProvider.deinit` wipes the whole `identity.key` union (covers the RSA variant identically to Ed25519/ECDSA) | wiped on `deinit`; the DER-parsing locals for `p`/`q`/`dp`/`dq`/`qInv` are stack copies with no dedicated wipe (see rationale) | modpow: constant-time-oriented by construction (see rationale); CRT-component validation at parse time: not constant-time (see rationale) | compliant, with a scoped deliberate limitation documented below | **Constant-time signing, no CRT**: the private operation is a direct `n`/`d` modular exponentiation via `std.crypto.ff.Modulus(4096).powWithEncodedExponent` — the stdlib's non-`public`-exponent path, which the same file's own verification path (`powWithEncodedPublicExponent`) deliberately does *not* use, precisely because `d` is secret. No CRT (`p`/`q`/`dp`/`dq`/`qInv`) is used for the operation, which sidesteps an entire class of CRT-specific side-channel and fault-injection concerns (CRT bit-flip/Bellcore-style attacks) at the cost of the ~4x speedup CRT would give; #565 accepted that trade-off explicitly for a first native implementation. **CRT components are validated, not retained, and not constant-time to validate**: `parsePrivateKeyDer` requires `p`, `q`, `dp`, `dq`, `qInv` to each be present, well-formed DER integers, and less than the modulus, but does **not** cross-check `p·q == n` or the `dp ≡ d (mod p−1)` / `dq ≡ d (mod q−1)` congruences — implementing that arbitrary-precision math correctly and re-reviewing it was judged higher-risk than the property it buys, since the signing path never touches these components: a key whose CRT components are inconsistent with `(n, d)` still signs exactly as `(n, d)` alone dictate, and a transcription error in `n` or `d` itself is self-detecting (the resulting signature fails verification, including the native verifier and, per the OpenSSL differential row in `docs/CRYPTO_SECURITY_AUDIT.md`'s sibling `tests/crypto_openssl_diff.zig`, an independent EVP verifier). This narrows the acceptance criterion "validate ... CRT components" to structural/range validation rather than full mathematical consistency — recorded here per #565's explicit instruction to document toolchain/scope limitations honestly rather than leave them implicit. Follow-up: if CRT-based signing is ever added for performance, the congruence checks above become load-bearing and must be added at that point, not before. |

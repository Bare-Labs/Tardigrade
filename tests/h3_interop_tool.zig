...
fn logEvent(_: ?*anyopaque, event: connection.Event) void {
    ...
    const line = switch (event) {
        ...
        .packets_acked => |p| std.fmt.bufPrint(&buf, "ack space={s} largest={d} count={d}", .{ @tagName(p.space), p.largest_acknowledged, p.acked_count }),
        ...
        .flow_control_blocked => |f| std.fmt.bufPrint(&buf, "flow_blocked scope={s} stream_id={?d}", .{ @tagName(f.scope), f.stream_id }),

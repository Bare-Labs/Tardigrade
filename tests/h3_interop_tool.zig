...
fn logEvent(_: ?*anyopaque, event: connection.Event) void {
    ...
    const line = switch (event) {
        ...
        .packets_acked => |p| std.fmt.bufPrint(&buf, "ack space={s} type={?s} largest={d} count={d}", .{ @tagName(p.space), if (p.packet_type) |kind| @tagName(kind) else null, p.largest_acknowledged, p.acked_count }),
        ...
        .stream_state_changed => |s| std.fmt.bufPrint(&buf, "stream_state id={d} old={?s} new={s}", .{ s.id, if (s.old) |old| @tagName(old) else null, @tagName(s.new) }),
